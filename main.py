...
# ======== CONFIG ========
workspaceName = "PPEPLNNusaDaya"
input_folder = "datasetsInput/PPEPLNNusaDaya-2"
...
CLASSLIST = ["Boots", "Gloves", "Mask", "No-Boots", "No-Gloves", "No-Mask", "Safety-Helmet", "Safety-Vest", "Safety-Wearpack", "Worker"]
...
def draw_all(frame_draw):
    for i,(x1,y1,x2,y2,cls) in enumerate(bboxes):
        color=(60,200,60) if i!=selected_bbox else (60,60,200)
        cv2.rectangle(frame_draw,(x1,y1),(x2,y2),color,1)
        cv2.putText(frame_draw,cls,(x1,y1-5),cv2.FONT_HERSHEY_SIMPLEX,0.6,color,1)
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if drawing:
            ...
            if w >= 5 and h >= 5:
                ...
                print("[INFO] Skipped tiny bbox (<5px).")
...
def draw_class_window():
    canvas=np.zeros((CLASS_WINDOW_H+60,CLASS_WINDOW_W,3),dtype=np.uint8)
    ...
    for i in range(start_idx,end_idx):
        ...
        text=f"{CLASSLIST[i]} ({class_counts[CLASSLIST[i]]})"
        cv2.putText(canvas,text,(10,y_pos+22),cv2.FONT_HERSHEY_SIMPLEX,0.6,(255,255,255),1)
        y_pos+=CLASS_HEIGHT

    total_images=len(images)
    info_text=f"Image {current_index+1}/{total_images}"
    cv2.rectangle(canvas,(0,CLASS_WINDOW_H),(CLASS_WINDOW_W,CLASS_WINDOW_H+60),(40,40,40),-1)
    ...
    for r in results:
        if hasattr(r,"boxes"):
            for box in r.boxes:
                ...
                x1_disp=int(x1*display_scale);y1_disp=int(y1*display_scale)
                x2_disp=int(x2*display_scale);y2_disp=int(y2*display_scale)
...
while True:
    ...
    while True:
        ...
        if key==ord('d'):
            ...
            prev_bboxes = [b.copy() for b in bboxes]
            current_index=(current_index+1)%len(images)
            break
        elif key==ord('a'):
            save_pascal_voc(img_name,orig_shape)
